/*
This file contains functions for drawing 2d primitives with a handy sketchy look
in p5.js.
...
written by Jo Wood, giCentre, City University London based on an idea by
Nikolaus Gradwohl. The handy library is licensed under the GNU Lesser General
...
function Scribble(p) {
  ...
  this.getOffset =
    ...
    this.sketch.endShape();
          ...
          this.getIntersectingLines =
              function(lineCoords, xCoords, yCoords) {
    var intersections = [];
    var s1 =
        new Segment(lineCoords[0], lineCoords[1], lineCoords[2], lineCoords[3]);

    for (var i = 0; i < xCoords.length; i++) {
      var s2 = new Segment(
          xCoords[i], yCoords[i], xCoords[(i + 1) % xCoords.length],
    ...
    this.sketch.curveVertex(
        ...
        y1 + this.getOffset(-offset, offset));
    this.sketch.curveVertex(
        midDispX + x1 + (x2 - x1) * divergePoint +
            this.getOffset(-offset, offset),
        midDispY + y1 + (y2 - y1) * divergePoint +
            this.getOffset(-offset, offset));
    this.sketch.curveVertex(
        midDispX + x1 + 2 * (x2 - x1) * divergePoint +
            this.getOffset(-offset, offset),
        midDispY + y1 + 2 * (y2 - y1) * divergePoint +
            this.getOffset(-offset, offset));
    this.sketch.curveVertex(
    ...
    this.sketch.bezier(
        x1 + this.getOffset(-2, 2), y1 + this.getOffset(-2, 2),
    ...
    this.scribbleCurve(
        left + radius, top, left, top + radius, left + radius * 0.1,
        top + radius * 0.1, left + radius * 0.1, top + radius * 0.1);
    this.scribbleCurve(
        right - radius, top, right, top + radius, right - radius * 0.1,
        top + radius * 0.1, right - radius * 0.1, top + radius * 0.1);
    this.scribbleCurve(
        left + radius, bottom, left, bottom - radius, left + radius * 0.1,
        bottom - radius * 0.1, left + radius * 0.1, bottom - radius * 0.1);
    this.scribbleCurve(
        right - radius, bottom, right, bottom - radius, right - radius * 0.1,
        bottom - radius * 0.1, right - radius * 0.1, bottom - radius * 0.1);
  }

                              this.scribbleEllipse =
                                  function(x, y, w, h) {
    var rx = Math.abs(w / 2);
    var ry = Math.abs(h / 2);
    ...
    this.buildEllipse(x, y, rx, ry, 1.5, 0);
                                  ...
                                  this.scribbleFilling = function(
                                      xCoords, yCoords, gap, angle) {
    if ((xCoords == null) || (yCoords == null) || (xCoords.length == 0) ||
        (yCoords.length == 0)) {
      return;
    }
    ...
    var it = new HachureIterator(
        top - 1, bottom + 1, left - 1, right + 1, gap, sinAngle, cosAngle,
        tanAngle);
    var rectCoords = null;
...
function HachureIterator(
    _top, _bottom, _left, _right, _gap, _sinAngle, _cosAngle, _tanAngle) {
  ...
  this.compare =
    ...
    if (grad1 == Number.MAX_VALUE) {
      ...
      if (Math.abs(otherSegment.getA()) < 0.00001) {
        if ((otherSegment.getPx1() - xi) * (xi - otherSegment.getPx2()) <
            -0.00001) {
          return Relation.SEPARATE;
        }
        return Relation.INTERSECTS;
    ...
    if (grad2 == Number.MAX_VALUE) {
      ...
      if (((otherSegment.getPy1() - yi) * (yi - otherSegment.getPy2()) <
           -0.00001) ||
          ((y1 - yi) * (yi - y2) < -0.00001)) {
        return Relation.SEPARATE;
      }
    ...
    return c;
                                      ...
                                      this.getIntersectionX =
                                          function() {
    return xi;
  }

                                          this.getIntersectionY =
                                              function() {
    return yi;
  }
